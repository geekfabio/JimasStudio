    ...
    <section class="pt-32 pb-16 px-6">
        <div class="max-w-7xl mx-auto">
            <div class="text-center max-w-2xl mx-auto mb-16">
                <span class="section-label inline-block mb-4">Portfólio</span>
                <h1 class="font-display font-bold text-4xl md:text-5xl text-ink-50 mb-4">Trabalhos realizados</h1>
                <p class="text-ink-300">Projectos que reflectem a nossa criatividade, estratégia e compromisso com resultados.</p>
            </div>

            @if ($items->isNotEmpty())
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($items as $item)
                        <a href="{{ route('portfolio.show', $item->slug) }}" class="group block relative rounded-2xl overflow-hidden h-80">
                            ...
                            <div class="absolute inset-0 bg-gradient-to-t from-ink-900/90 to-transparent"></div>
                            <div class="absolute bottom-0 left-0 p-6">
                                <p class="text-gold-300 text-xs uppercase tracking-wider mb-1">{{ $item->category ?? 'Projecto' }}</p>
                                <h3 class="font-display font-bold text-xl text-white">{{ $item->title }}</h3>
                                @if ($item->client)
                                    <p class="text-ink-400 text-sm mt-1">{{ $item->client }}</p>
                                @endif
                            </div>
                        </a>
                    @endforeach
