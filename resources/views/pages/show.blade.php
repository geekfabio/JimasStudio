...
@section('content')
    ...
    <section class="relative pt-28 pb-16 md:pt-32 md:pb-20 px-4 sm:px-6 overflow-hidden">
        ...
        <div class="relative z-10 max-w-4xl mx-auto text-center px-2">
            <span class="section-label inline-block mb-3 md:mb-4">Sobre Nós</span>
            <h1 class="font-display font-extrabold text-3xl sm:text-4xl md:text-6xl text-ink-50 mb-4 md:mb-6 leading-tight">
                {{ $page->title }}
            </h1>
            <p class="text-ink-300 text-base sm:text-lg md:text-xl max-w-2xl mx-auto">
    ...
    <!-- Secções em cards -->
    @if (count($sections) > 0)
        <section class="py-8 md:py-16 px-4 sm:px-6 bg-ink-50/30">
                    ...
                    @foreach ($sections as $section)
                        ...
                        @if ($isWhyUs)
                            <div class="md:col-span-2">
                                <div class="bg-white rounded-2xl p-6 sm:p-8 md:p-10 shadow-sm border border-ink-700/10">
                                    <h2 class="font-display font-bold text-2xl sm:text-3xl text-ink-50 mb-4 sm:mb-6 text-center">{{ $section['title'] }}</h2>
                                    <div class="prose prose-sm sm:prose-base max-w-none text-ink-300 leading-relaxed">
                                        {!! $section['body'] !!}
                                    </div>
                                </div>
                            </div>
                        @else
                            <div class="bg-white rounded-2xl p-5 sm:p-6 md:p-8 shadow-sm border border-ink-700/10">
                                <h2 class="font-display font-bold text-xl sm:text-2xl text-ink-50 mb-3 sm:mb-4">{{ $section['title'] }}</h2>
                                <div class="prose prose-sm sm:prose-base max-w-none text-ink-300 leading-relaxed">
                                    {!! $section['body'] !!}
                                </div>
                            </div>
    ...
    <section class="py-16 md:py-24 px-4 sm:px-6 text-center relative overflow-hidden">
        <div class="absolute inset-0 opacity-5" style="background-image: repeating-linear-gradient(45deg, #27b3cb 0, #3B82F6 1px, transparent 0, transparent 50%); background-size: 20px 20px;"></div>
        <div class="relative z-10 max-w-2xl mx-auto px-2">
            ...
            <a href="{{ $whatsappUrl }}" target="_blank" class="wa-btn inline-flex items-center justify-center gap-2 sm:gap-3 px-6 sm:px-8 py-3 sm:py-4 rounded-full text-white font-semibold text-base sm:text-lg shadow-lg">
