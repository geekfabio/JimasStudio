...
@section('content')
    @php
        ...
        $heroImage = setting('home_hero_image');
        $aboutImage = setting('home_about_image');
        $newsFallback = setting('home_news_fallback_image');
    @endphp

    <!-- Hero -->
    <section class="min-h-screen flex flex-col items-center justify-center relative overflow-hidden pt-28 pb-20 md:pt-32 md:pb-24 px-4 sm:px-6">
        <div class="absolute inset-0">
            <img src="{{ $heroImage ? asset('storage/'.$heroImage) : 'https://images.unsplash.com/photo-1556761175-5973dc0f32e7?w=1920&h=1080&fit=crop&auto=format&q=80' }}" alt="{{ setting('home_hero_title', 'JIMAS Marketing e Comunicação') }}" fetchpriority="high" class="w-full h-full object-cover" />
        ...
        <div class="relative z-10 text-center max-w-5xl mx-auto">
            <span class="section-label inline-block mb-5 md:mb-8 animate-fade-in-up" style="animation-delay: 0.1s;">{{ setting('home_hero_label', 'Marketing & Comunicação · Angola') }}</span>
            <h1 class="hero-title font-display font-extrabold tracking-tight mb-6 md:mb-8 text-ink-50 animate-fade-in-up" style="animation-delay: 0.2s;">
                {!! nl2br(e(setting('home_hero_title', 'Mais do que marketing, entregamos crescimento.'))) !!}
            </h1>
            ...
            <div class="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4 animate-fade-in-up" style="animation-delay: 0.4s;">
                <a href="{{ route('servicos') }}" class="cursor-pointer px-6 py-3.5 sm:px-8 sm:py-4 rounded-full text-white bg-gold-300 hover:bg-brand-turquoise transition-all duration-200 font-semibold shadow-md hover:shadow-lg hover:-translate-y-0.5">
                    {{ setting('home_hero_primary_text', 'Ver serviços') }}
                </a>
                <a href="{{ $whatsappUrl }}" target="_blank" class="cursor-pointer px-6 py-3.5 sm:px-8 sm:py-4 rounded-full border-2 border-ink-600 text-ink-200 hover:border-brand-turquoise hover:text-brand-turquoise transition-all duration-200 font-semibold">
                    {{ setting('home_hero_secondary_text', 'Falar connosco') }}
                </a>
            </div>
        </div>
    </section>
    ...
    @if ($aboutPage)
        <section class="py-20 md:py-28 px-4 sm:px-6">
            <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
                <div class="reveal">
                    <span class="section-label inline-block mb-5">{{ setting('home_about_label', 'Quem Somos') }}</span>
                    <h2 class="font-display font-bold text-3xl sm:text-4xl md:text-5xl text-ink-50 mb-6 md:mb-8 leading-tight">{{ setting('home_about_title', $aboutPage->title) }}</h2>
                    <div class="prose prose-base sm:prose-lg max-w-none text-ink-300 leading-relaxed line-clamp-6">
                        {!! setting('home_about_text', $aboutPage->content) !!}
                    </div>
                    <a href="{{ route('pages.show', $aboutPage->slug) }}" class="cursor-pointer inline-flex items-center gap-2 mt-8 md:mt-10 px-6 py-3.5 sm:px-8 sm:py-4 rounded-full border-2 border-ink-600 text-ink-200 hover:border-gold-300 hover:text-gold-300 transition-all duration-200 font-semibold">
                        {{ setting('home_about_button', 'Saber mais') }}
                ...
                <div class="relative rounded-3xl overflow-hidden h-80 sm:h-[28rem] shadow-xl reveal reveal-delay-2">
                    <img src="{{ $aboutImage ? asset('storage/'.$aboutImage) : 'https://images.unsplash.com/photo-1552664730-d307ca884978?w=800&h=600&fit=crop&auto=format&q=80' }}" alt="{{ setting('home_about_title', 'Equipa JIMAS') }}" loading="lazy" class="w-full h-full object-cover" />
                </div>
            </div>
        </section>
    @endif
        ...
        <section class="py-20 md:py-28 px-4 sm:px-6 bg-brand-dark">
            <div class="max-w-7xl mx-auto">
                <div class="text-center max-w-2xl mx-auto mb-12 md:mb-20 reveal">
                    <span class="section-label inline-block mb-5">{{ setting('home_services_label', 'Nossos Serviços') }}</span>
                    <h2 class="font-display font-bold text-3xl sm:text-4xl md:text-5xl text-white mb-5">{{ setting('home_services_title', 'Da estratégia à execução') }}</h2>
                    <div class="text-white/70 text-base sm:text-lg">{!! setting('home_services_text', 'Cobrimos todas as áreas para posicionar a tua empresa como referência no mercado.') !!}</div>
                </div>
        ...
        <section class="py-20 md:py-28 px-4 sm:px-6">
            <div class="max-w-7xl mx-auto">
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6 mb-10 md:mb-16 reveal">
                    <div>
                        <span class="section-label inline-block mb-3 md:mb-5">{{ setting('home_portfolio_label', 'Portfólio') }}</span>
                        <h2 class="font-display font-bold text-3xl sm:text-4xl md:text-5xl text-ink-50">{{ setting('home_portfolio_title', 'Trabalhos seleccionados') }}</h2>
                    </div>
    ...
    @if ($news->isNotEmpty())
        <section class="py-20 md:py-28 px-4 sm:px-6 bg-brand-dark">
            <div class="max-w-7xl mx-auto">
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6 mb-10 md:mb-16 reveal">
                    <div>
                        <span class="section-label inline-block mb-3 md:mb-5">{{ setting('home_news_label', 'Notícias') }}</span>
                        <h2 class="font-display font-bold text-3xl sm:text-4xl md:text-5xl text-white">{{ setting('home_news_title', 'Últimas actualizações') }}</h2>
                    </div>
                    ...
                    @foreach ($news as $item)
                        <article class="reveal group bg-white/5 backdrop-blur-sm rounded-2xl overflow-hidden border border-white/10 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-200">
                            <a href="{{ route('news.show', $item->slug) }}" class="cursor-pointer block relative h-48 sm:h-52 overflow-hidden">
                                <img src="{{ $item->cover_image ? asset('storage/' . $item->cover_image) : ($newsFallback ? asset('storage/'.$newsFallback) : 'https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=600&h=350&fit=crop&auto=format&q=80') }}" alt="{{ $item->title }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                            </a>
        ...
        <div class="relative z-10 max-w-7xl mx-auto">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-center">
                ...
                <div class="text-center lg:text-left reveal">
                    <span class="section-label inline-block mb-5 md:mb-6">{{ setting('home_cta_label', 'Vamos trabalhar juntos') }}</span>
                    <h2 class="font-display font-bold text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-ink-50 mb-5 md:mb-6 leading-tight">{{ setting('home_cta_title', 'Pronto para transformar a tua empresa?') }}</h2>
                    <div class="text-ink-300 text-base sm:text-lg md:text-xl mb-8 md:mb-10 max-w-xl mx-auto lg:mx-0">{!! setting('home_cta_text', 'Envia-nos uma mensagem ou fala connosco pelo WhatsApp. A JIMAS ajuda-te a posicionar o teu negócio como referência no mercado.') !!}</div>
                    <a href="{{ $whatsappUrl }}" target="_blank" class="cursor-pointer wa-btn inline-flex items-center justify-center gap-3 px-8 py-4 sm:px-10 sm:py-5 rounded-full text-white font-semibold text-base sm:text-lg shadow-lg hover:shadow-xl transition-all duration-200 hover:-translate-y-0.5">
                        ...
                        {{ setting('home_cta_button', 'Falar no WhatsApp') }}
                    </a>
                </div>
