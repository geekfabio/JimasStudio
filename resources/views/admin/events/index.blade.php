...
@section('content')
    <div class="flex items-center justify-between mb-8">
        <h1 class="font-display font-bold text-3xl text-ink-50">Eventos</h1>
        <a href="{{ route('admin.events.create') }}" class="px-5 py-2.5 rounded-lg bg-gold-300 hover:bg-gold-200 text-ink-50 font-semibold transition-colors">Novo evento</a>
    </div>
        ...
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-ink-200">
                @forelse ($events as $event)
                    <tr>
                        <td class="px-6 py-4 text-ink-50">{{ $event->title }}</td>
                        <td class="px-6 py-4 text-ink-500">{{ $event->event_date?->format('d/m/Y H:i') ?? '-' }}</td>
                        <td class="px-6 py-4 text-ink-500">{{ $event->location ?? '-' }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            <a href="{{ route('admin.events.edit', $event) }}" class="text-gold-300 hover:underline">Editar</a>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-ink-500">Nenhum evento registado.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
