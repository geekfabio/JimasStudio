<form method="POST" action="{{ $route }}" enctype="multipart/form-data" class="bg-white border border-ink-200 rounded-xl p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            <label class="block text-sm font-medium text-ink-500 mb-1">Data do evento</label>
        ...
        <div>
            <label class="block text-sm font-medium text-ink-500 mb-1">Local</label>
        ...
        <div class="md:col-span-2">
            ...
            <textarea name="description" rows="5"
                class="w-full px-4 py-2 rounded-lg bg-white border border-ink-200 text-ink-50 focus:border-gold-300 outline-none">{{ old('description', $event?->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            @if ($event?->cover_image)
                <img src="{{ asset('storage/' . $event->cover_image) }}" alt="" class="mt-3 h-32 rounded-lg object-cover" />
            @endif
        </div>
