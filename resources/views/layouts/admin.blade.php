...
<head>
    ...
    <style>
        ...
        .admin-sidebar details > div > a.active {
            background: rgba(39, 179, 203, 0.10);
            color: #27b3cb;
        }
    </style>
    ...
    <div class="min-h-screen flex">
        <aside class="w-64 bg-white border-r border-ink-200 flex flex-col">
            ...
            <nav class="flex-1 p-4 space-y-1 admin-sidebar" aria-label="Menu principal">
                ...
                <details class="group" {{ request()->routeIs('admin.services.*') ? 'open' : '' }}>
                    <summary class="flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer font-medium text-ink-400 hover:bg-ink-800 hover:text-ink-50 transition-colors list-none {{ request()->routeIs('admin.services.*') ? 'bg-gold-300/10 text-gold-300' : '' }}">
                        ...
                        <span class="flex-1">Serviços</span>
                    ...
                    <div class="ml-4 mt-1 space-y-1 border-l-2 border-ink-700 pl-4">
                        <a href="{{ route('admin.services.index') }}" class="block px-4 py-2 rounded-lg text-sm {{ request()->routeIs('admin.services.index', 'admin.services.create', 'admin.services.edit') ? 'text-gold-300 bg-gold-300/10 font-medium' : 'text-ink-400 hover:text-ink-50 hover:bg-ink-800' }}">
                            Categorias
                        </a>
                        <a href="{{ route('admin.services.plans.index') }}" class="block px-4 py-2 rounded-lg text-sm {{ request()->routeIs('admin.services.plans.*') ? 'text-gold-300 bg-gold-300/10 font-medium' : 'text-ink-400 hover:text-ink-50 hover:bg-ink-800' }}">
                            Planos
                        </a>
                    </div>
                </details>
